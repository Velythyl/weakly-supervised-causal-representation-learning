...
def next_step_interventions(prev_time_step, settings, ball_reset=False):
    ...
    if not ball_reset:
        new_time_step['paddle_left_y'] = paddle_step(prev_time_step['paddle_left_y'], prev_time_step['ball_y'],
                                                     settings, intervention=True)
        new_time_step['paddle_right_y'] = paddle_step(prev_time_step['paddle_right_y'], prev_time_step['ball_y'],
                                                      settings, intervention=True)
    else:
        new_time_step['paddle_left_y'] = np.random.uniform(settings['center_point_y'] * 0.25,
                                                           settings['center_point_y'] * 1.75)
        new_time_step['paddle_right_y'] = np.random.uniform(settings['center_point_y'] * 0.25,
                                                            settings['center_point_y'] * 1.75)
...
def next_step_regular(prev_time_step, settings):
    ...
    if ball_reset:
        ...
        if ball_collision('paddle_left', new_time_step, prev_time_step, settings):
            new_time_step['ball_x'] = (settings['paddle_left_x'] + settings['paddle_width'] / 2.0) * 2 - (
                        new_time_step['ball_x'] - settings['ball_radius'] * 2)
            ...
            new_time_step['ball_x'] = (settings['paddle_right_x'] - settings['paddle_width'] / 2.0) * 2 - (
                        new_time_step['ball_x'] + settings['ball_radius'] * 2)
            new_time_step['ball_vel_dir'] = angle_flip(new_time_step['ball_vel_dir'], axis='y')
        # Noise addition
...
def create_settings():
    ...
    settings['ball_x_max_sample'] = settings['paddle_right_x'] - settings['paddle_width'] / 2.0 - settings[
        'ball_radius']
...
def plot_matplotlib_figure(time_step, settings, filename):
    ...
    paddle_left = plt.Rectangle((settings['paddle_left_x'] - settings['paddle_width'] / 2.0,
                                 time_step['paddle_left_y'] - settings['paddle_height'] / 2.0),
                                settings['paddle_width'], settings['paddle_height'], fc='b', snap=False)
    ax.add_patch(paddle_left)
    paddle_right = plt.Rectangle((settings['paddle_right_x'] - settings['paddle_width'] / 2.0,
                                  time_step['paddle_right_y'] - settings['paddle_height'] / 2.0),
                                 settings['paddle_width'], settings['paddle_height'], fc='g', snap=False)
    ...
    border_right = plt.Rectangle((settings['resolution'] - settings['border_size'], 0), settings['border_size'],
                                 settings['resolution'], fc=np.array([0.1, 0.1, 0.1]))
    ...
    border_top = plt.Rectangle((0, settings['resolution'] - settings['border_size']), settings['resolution'],
                               settings['border_size'], fc=np.array([0.1, 0.1, 0.1]))
...
def export_figures(folder, start_index=0, end_index=-1):
    ...
    figures = np.zeros((end_index - start_index, settings['resolution'] * settings['dpi'],
                        settings['resolution'] * settings['dpi'], 4), dtype=np.uint8)
