...
from experiments_minimal.experiment_utils import (
...
from ws_crl_minimal.encoder import GaussianEncoder
...
@hydra.main(config_path="../config_minimal", config_name="causalcircuit_ilcm")
def main(cfg):
...
def get_dataloader(cfg, tag, batchsize=None, shuffle=False, include_noise_encodings=False):
    ...
    try:
        ...
        if len(data) > 6 and isinstance(data[6], list):
            data = list(data)
            data[6] = torch.cat(data[6], dim=0)
            data[7] = torch.cat(data[7], dim=0)
